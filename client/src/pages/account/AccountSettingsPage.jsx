...
export function AccountSettingsPage() {
  ...
  const [notificationsEnabled, setNotificationsEnabled] = useState(() => {
    ...
    return localStorage.getItem("madvira_notifications") !== "off";
  });

  const toggleNotifications = () => {
    setNotificationsEnabled((prev) => {
      const next = !prev;
      localStorage.setItem("madvira_notifications", next ? "on" : "off");
      return next;
    });
  };
        ...
        <div className="bg-[#171717] border border-[#262626] rounded-2xl p-8 shadow-xl">
          ...
          <div className="space-y-4">
            <button
              type="button"
              onClick={toggleTheme}
              className="w-full flex items-center justify-between rounded-xl border border-[#303030] px-4 py-3 text-sm text-white hover:bg-[#202020]"
            >
            ...
            >
              ...
              <span className="text-muted">{notificationsEnabled ? "On" : "Off"}</span>
            </button>
          </div>
        </div>
