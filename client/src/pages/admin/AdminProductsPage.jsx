...
export function AdminProductsPage() {
  ...
  const totalStock = products.reduce(
    (s, p) => s + (p.sizes || []).reduce((vs, v) => vs + (v.stock || 0), 0),
    0
  );
  const hiddenCount = products.filter((p) => !p.isActive).length;
  const newDropCount = products.filter((p) => p.isNewCollection).length;
        ...
        {loading ? (
          ...
          <>
              ...
              <table className="w-full text-sm min-w-[860px]">
                <thead>
                  <tr className="border-b border-[#1e1e1e]">
                    ...
                    {["Product", "Price", "Stock", "Visibility", "New Collection", "Upcoming", ""].map((h) => (
                      <th
                        key={h}
                    ...
                    {products.map((p, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-3.5">
                            <span
                              className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold border ${p.isActive
                                ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/25"
                                : "bg-zinc-800/60 text-zinc-500 border-zinc-700/60"
                                }`}
                            >
                              <span
                                className={`w-1.5 h-1.5 rounded-full ${p.isActive ? "bg-emerald-400" : "bg-zinc-600"
                                  }`}
                              />
                          ...
                          <td className="px-4 py-3.5">
                            {p.isUpcoming ? (
                              <span className="inline-flex items-center gap-1 rounded-full bg-red-500/10 border border-red-500/25 px-2.5 py-1 text-[11px] font-semibold text-red-400">
                                Coming Soon
                              </span>
                            ) : (
                              <span className="text-[11px] text-zinc-700">—</span>
                          ...
                          <td className="px-4 py-3.5">
                            <div className="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                              ...
                              <button
                                ...
                                className="rounded-lg border border-[#2a2a2a] bg-[#1a1a1a] px-3 py-1.5 text-[11px] font-medium text-zinc-300 hover:text-white hover:border-zinc-600 hover:bg-[#222] transition-all duration-150"
                              >
                                {p.isNewCollection ? "Unmark New" : "Mark New"}
                              </button>
                              <button
                                onClick={async () => {
                                  await api.patch(`/admin/products/${p._id}/featured`);
                                  loadProducts();
                                }}
                                className="rounded-lg border border-[#2a2a2a] bg-[#1a1a1a] px-3 py-1.5 text-[11px] font-medium text-zinc-300 hover:text-white hover:border-zinc-600 hover:bg-[#222] transition-all duration-150"
                              >
                                {p.isUpcoming ? "Unmark Upcoming" : "Mark Upcoming"}
                              </button>
                              <button
            ...
            <div className="md:hidden divide-y divide-[#1c1c1c]">
              {products.map((p, i) => {
                const stock = (p.sizes || []).reduce((s, v) => s + (v.stock || 0), 0);
                return (
                  <motion.div
                    key={p._id}
                    initial={{ opacity: 0 }}
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm font-bold text-accent mt-0.5">₹{(p.basePrice || 0).toFixed(2)}</p>
                      </div>
                    </div>
                    <div className="flex flex-wrap gap-2 ml-10">
                      <StockBadge stock={stock} />
                      <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-semibold border ${p.isActive
                        ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/25"
                        : "bg-zinc-800/60 text-zinc-500 border-zinc-700/60"
                        }`}>
                        {p.isActive ? "Visible" : "Hidden"}
                      ...
                      {p.isUpcoming && (
                        <span className="inline-flex items-center rounded-full bg-red-500/10 border border-red-500/25 px-2.5 py-1 text-[11px] font-semibold text-red-400">
                          Coming Soon
                        </span>
                      )}
                    </div>
