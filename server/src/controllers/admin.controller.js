...
const DEFAULT_PRODUCT_IMAGE =
  "https://via.placeholder.com/1200x1500?text=Product+Image";

function normalizeProductPayload(payload) {
  const next = { ...payload };

  if (typeof next.visible === "boolean") next.isActive = next.visible;
  if (typeof next.newDrop === "boolean") next.isNewCollection = next.newDrop;

  next.basePrice = Number(next.basePrice ?? next.price) || 0;
  next.images = Array.isArray(next.images)
    ? next.images.filter(Boolean).slice(0, 6)
    : [];
  if (next.images.length === 0) {
    next.images = Array(6).fill(DEFAULT_PRODUCT_IMAGE);
  } else {
    while (next.images.length < 6) {
      next.images.push(next.images[0]);
    }
  }
  ...
  next.sizes = (next.sizes || [])
    ...
    .filter((entry) => ["S", "M", "L", "XL"].includes(entry.size));

  const existingBySize = new Map(
    next.sizes.map((entry) => [entry.size, entry]),
  );
  next.sizes = ["S", "M", "L", "XL"].map(
    (size) => existingBySize.get(size) || { size, stock: 0 },
  );
  next.sizeChart = {
    text: String(next.sizeChart?.text || next.sizeChart?.notes || "").trim(),
    image: String(next.sizeChart?.image || "").trim(),
  };
  next.washCare = String(next.washCare || "").trim();
  next.isActive = next.isActive !== false;
  next.isNewCollection = Boolean(next.isNewCollection);
  next.isUpcoming = Boolean(next.isUpcoming);
  delete next.visible;
  delete next.newDrop;
  delete next.price;
...
export async function adminListProducts(req, res) {
  ...
  if (visibility === "visible") filters.isActive = true;
  if (visibility === "hidden") filters.isActive = false;
  if (newDrop === "true") filters.isNewCollection = true;
...
export async function adminMarkFeatured(req, res) {
  ...
  product.isUpcoming = !product.isUpcoming;
  await product.save();
  res.json(product);
}
...
export async function adminUpdateProductStock(req, res) {
  ...
  if (!size || Number.isNaN(newCount)) {
    res.status(400);
    throw new Error("Invalid size stock payload");
  }

  const sizeEntry = product.sizes.find(
    (entry) => entry.size === String(size).toUpperCase(),
  );
  if (!sizeEntry) {
    res.status(404);
